...
Therefore the codex pipeline is a **top-level CLI group** (`imas-codex mint`), not nested under `imas`. It sits alongside `discover` and `imas` as a peer command group.
...
│                imas-codex                     │
│                                              │
│  NEW: mint/ module (top-level, NOT under imas)│
...
│    imas-codex mint build --source dd          │
│    imas-codex mint build --source signals     │
│    imas-codex mint build --ids equilibrium    │
│    imas-codex mint status                     │
│    imas-codex mint benchmark                  │
│                                              │
│  Graph nodes:                                │
...
**Revised resolution:** The pipeline mints from **multiple sources** (DD paths, facility signals, potentially others), so it is not IMAS-specific. Create a new **top-level `mint` module** in codex (`imas_codex/mint/`) with source plugins. CLI becomes `imas-codex mint build --source dd` or `imas-codex mint build --source signals`. This sits alongside `discover` and `imas` as a peer command group.
...
**CLI placement in codex:** `imas-codex mint` — a top-level command group, not nested under `imas` or `discover`.
...
1. `imas_codex/mint/` — new top-level module with source plugins
2. `imas_codex/mint/sources/dd.py` — extract from IMASNode paths
3. `imas_codex/mint/sources/signals.py` — extract from FacilitySignal nodes
4. `imas_codex/cli/mint.py` — top-level `mint` CLI group
5. `llm/prompts/mint/` — Jinja2 generation/review templates
