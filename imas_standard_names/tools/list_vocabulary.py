...
class VocabularyTokensTool(CatalogTool):
    ...
    @mcp_tool(
        description=(
            "Get vocabulary tokens for IMAS Standard Names grammar segments. "
            "Returns complete token lists with metadata, templates, validation rules, "
            "and optional usage statistics from existing catalog entries. "
            "Use this to discover available tokens before composing names or "
            "to analyze vocabulary usage patterns. "
            "Supports filtering by specific segment (component, subject, etc.) or "
            "returns all vocabularies. Includes both enumerated vocabularies "
            "(component, subject, object, etc.) and open vocabularies (physical_base). "
            "Complements get_naming_grammar tool which provides grammar rules and overview."
        )
    )
