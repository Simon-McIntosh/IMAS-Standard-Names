...
We adopt an *absolute uniform* component naming pattern and encourage using the
...
derivative_with_respect_to_{coord} | any | same | yes | {coord} from dataset.
...
### 11.4 Gradients
`gradient_of_<scalar>` is a vector (kind: derived_vector). Do not create a
scalar `gradient_of_*` entry; instead generate its components.
