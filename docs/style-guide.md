...
harvest existing IMAS Data Dictionary information for equilibrium and
diagnostics before drafting new names. Align proposals with real data.
...
Numbers | Use plain digits without zero padding unless ordering benefits (e.g. `pf_coil_01` optional; keep consistent per set).
...
Time derivative | Prefix | `time_derivative_of_electron_temperature`
Gradient (vector) | Use derived vector rules | `gradient_of_electron_temperature`
Scalar from vector | Scalarizing operator prefix or magnitude suffix | `divergence_of_magnetic_field`, `magnetic_field_magnitude`
No frame terms | Omit coordinate hints | (avoid) `electron_temperature_radial`

Anti-patterns: `electron_temperature_time_derivative`, `magnitude_of_magnetic_field` (use suffix form), `gradient_of_electron_temperature_radial_component` (rank misuse).
...
Magnitude scalar | `<vector_expression>_magnitude` | `magnetic_field_magnitude`
...
`magnitude_of_magnetic_field` | Non-canonical prefix form | `magnetic_field_magnitude`
...
Magnitude: <vector_expression>_magnitude
